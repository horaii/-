...
const TARGET_LAT = 37.551694; 
const TARGET_LNG = 127.218528; 
...
btn.addEventListener('click', () => {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
            btn.disabled = false;
            let errorMsg = "위치 정보를 가져오는 데 실패했습니다.";
            if (error.code === error.PERMISSION_DENIED) {
                errorMsg = "위치 권한이 거부되었습니다. 브라우저 설정에서 위치 권한을 허용해 주세요.";
            } else if (error.code === error.TIMEOUT) {
                errorMsg = "위치 확인 시간이 초과되었습니다. 다시 시도해 주세요.";
            }
            statusDiv.innerHTML = `<span class='text-red-500'>${errorMsg}</span>`;
        },
